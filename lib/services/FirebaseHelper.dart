...
class FireStoreUtils {
  ...
  static Future<List<VehicleTypeModel>> getVehicleType(
    ...
    await Future.forEach(currencyQuery.docs,
        (QueryDocumentSnapshot<Map<String, dynamic>> document) {
      try {
        vehicleType.add(VehicleTypeModel.fromJson(document.data()));
      } catch (e) {
  ...
  static Future<List<VehicleTypeModel>> getRentalVehicleType() async {
    List<VehicleTypeModel> vehicleType = [];
    QuerySnapshot<Map<String, dynamic>> currencyQuery = await firestore
  ...
  static Future<List<VehicleMake>> getCarMakes([String? vehicleType]) async {
    List<VehicleMake> carMakesList = [];
    try {
      Query<Map<String, dynamic>> query =
          firestore.collection(CARMAKES).where("isActive", isEqualTo: true);
      ...
      QuerySnapshot<Map<String, dynamic>> currencyQuery = await query.get();

      await Future.forEach(currencyQuery.docs,
          (QueryDocumentSnapshot<Map<String, dynamic>> document) {
        try {
          carMakesList.add(VehicleMake.fromJson(document.data()));
        } catch (e) {
          print('FireStoreUtils.getCarMakes Parse error: $e');
        }
      });
    } catch (e) {
      print('FireStoreUtils.getCarMakes Error: $e');
    }

    return carMakesList;
  }

  static Future<List<VehicleTypeModel>> getVehicles() async {
    ...
    await Future.forEach(currencyQuery.docs,
        (QueryDocumentSnapshot<Map<String, dynamic>> document) {
      try {
        vehiclesList.add(VehicleTypeModel.fromJson(document.data()));
      } catch (e) {
        print('FireStoreUtils.getCurrencys Parse error $e');
      }
    });
  ...
  static Future<List<VehicleModel>> getCarModel(
      BuildContext context, String name) async {
    showProgress(context, 'Please wait...'.tr(), false);

    List<VehicleModel> carMakesList = [];
    QuerySnapshot<Map<String, dynamic>> currencyQuery = await firestore
        .collection(CARMODEL)
    ...
    await Future.forEach(currencyQuery.docs,
      ...
      try {
        print(document.data());
        carMakesList.add(VehicleModel.fromJson(document.data()));
      } catch (e) {
        print('FireStoreUtils.getCurrencys Parse error $e');
      }
    });
...
// if (docSnapshot.exists) {
  ...
  // Método para atualizar carteira de recarga
  static Future updateRechargeBalance(
      {required String userId, required amount}) async {
    dynamic rechargeBalance = 0;
    await firestore.collection(USERS).doc(userId).get().then((value) async {
      DocumentSnapshot<Map<String, dynamic>> userDocument = value;
      if (userDocument.data() != null && userDocument.exists) {
        try {
          ...
          await firestore.collection(USERS).doc(userId).update({
            "recharge_balance":
                (userDocument.data()!['recharge_balance'] ?? 0.0) + amount
          }).then((value) => print("recharge updated"));
          ...
          if (error.toString() ==
            ...
            print("went wrong!!");
            rechargeBalance = "ERROR";
          }
        }
        print("data val");
        print(rechargeBalance);
  ...
  // Método para atualizar carteira de ganhos
  static Future updateEarningsBalance(
      {required String userId, required amount}) async {
    dynamic earningsBalance = 0;
    await firestore.collection(USERS).doc(userId).get().then((value) async {
      DocumentSnapshot<Map<String, dynamic>> userDocument = value;
      if (userDocument.data() != null && userDocument.exists) {
        try {
          ...
          await firestore.collection(USERS).doc(userId).update({
            "earnings_balance":
                (userDocument.data()!['earnings_balance'] ?? 0.0) + amount
          }).then((value) => print("earnings updated"));
  ...
  static Future<SectionModel?> getSectionBySectionId(String sectionId) async {
    DocumentSnapshot<Map<String, dynamic>> sectionDocument =
        await firestore.collection(SECTION).doc(sectionId).get();
    return SectionModel.fromJson(sectionDocument.data()!);
  }
  ...
  static sendMailToAdmin(
    ...
    await sendMail(
        subject: subject, isAdmin: false, body: message, recipients: [toEmail]);
  }
  ...
  static Future<String?> _showSmsCodeDialog() async {
    // This should be implemented in your UI, e.g., in sign_up_screen.dart
    // For now, return null to indicate it needs implementation
    return null;
  }

  static Future<String?> uploadUserImageAndGetURL(
      File image, String fieldName, BuildContext context) async {
    try {
      Reference storageRef = storage.child(
          'images/drivers/${MyAppState.currentUser!.userID}/$fieldName-${DateTime.now().millisecondsSinceEpoch}.jpg');

      // Show upload progress
      ShowToastDialog.showLoader('Enviando foto...');
      ...
      uploadTask.snapshotEvents.listen((TaskSnapshot snapshot) {
        ...
        ShowToastDialog.showLoader(
            'Enviando foto... ${progress.toStringAsFixed(1)}%');
      });
      ...
      ShowToastDialog.showToast('Erro ao enviar foto: ${e.toString()}');
      return null;
    }
  }
  ...
  static firebaseSignUpWithEmailAndPassword(
    ...
    try {
      ...
      errorMessage = await firebaseCreateNewUser(user);
      if (errorMessage == null) {
        return user;
      } else {
        return null;
    ...
    firestore
      ...
      try {
        OrangeMoney payStackSettingData =
            OrangeMoney.fromJson(payStackData.data() ?? {});
    ...
    firestore
      ...
      try {
        MidTrans payStackSettingData =
            MidTrans.fromJson(payStackData.data() ?? {});
  ...
  static Future<void> updateOrderDriverId(
      {required String orderId,
      required String driverId,
      required String driverStatus}) async {
    await firestore
        .collection(ORDERS)
        .doc(orderId)
        .update({'driverID': driverId, 'driver_status': driverStatus});
  }
}
